...
| 产物 | 路径 | 说明 |
|------|------|------|
| 全过程报告 | `.result/2401.04146/full_report.md`（本文，main-agent step11 定稿后投递） | 本文 |
| 简报 | `.result/2401.04146/brief.md` | 给 PI |
| SKILL 建议 | `toEflow/2401.04146.skill-suggestion.md` | 自迭代输入 |
| 蓝图建议 | `toEflow/2401.04146.blueprint-suggestion.md` | 本次无需蓝图 |
| benchmark 条目 | `reproduction_test/mie/data/benchmark.yaml` | 首次创建，含 sr 超标 known/accepted 标注 |
| 代码 | `reproduction_test/mie/code/`（10 个脚本） | scattering.py 等 |
| 数据 | `reproduction_test/mie/data/`（9 个 CSV） | loci CSV × 6 + 数字化 + Layer3 metrics + 扰动 |
| 图 | `reproduction_test/mie/figures/`（5 张） | 复现图/叠图/contour核对/分布直方图 |
| capsule | `.work/.result/0703-01-akimov-mie-v1/capsule.md`（step11 产出） | E-flow 唯一输入 |
