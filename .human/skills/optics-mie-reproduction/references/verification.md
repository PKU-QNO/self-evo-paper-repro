...
| 1.5 | Large-size extinction paradox | $Q_{ext}\to2$ as $x\to\infty$ | Large opaque or weakly absorbing sphere; enough multipole orders retained | **trend criterion**: $Q_{ext}$ monotonically approaches 2; end-point $x=800$ gives $\|Q_{ext}-2\|<0.05$（无损球慢收敛，单点达 0.05 需 $x\gtrsim1000$；弱阻尼压不掉慢收敛主偏离，只衰减 ripple） | trend not monotone / end-point $\gg0.05$ → structural error (漏 $b_l$、系数错半); single-point ripple ≤0.02 正常 | Strongly absorbing spheres ($Q_{ext}$ 不趋 2); nonspherical/periodic systems |
